...
const App: React.FC = () => {
  ...
  const startGeneration = useCallback(async (customPrompt?: string) => {
    ...
    const initialArtifacts: Artifact[] = [
      { id: 'art-1', title: 'Technical Architecture', type: 'architecture', content: '', status: 'streaming' },
      { id: 'art-2', title: 'User Interface Concept', type: 'ui', content: '', status: 'streaming' },
      { id: 'art-3', title: 'Data Flow & Logic', type: 'logic', content: '', status: 'streaming' }
    ];
        ...
        setSession(prev => {
          ...
          return {
            ...
            artifacts: prev.artifacts.map(a => 
              a.id === art.id ? { ...a, content: result.text || 'Error generating content', status: 'complete' } : a
            )
          };
        });
      });
  ...
  const addAgent = async () => {
    ...
    const newAgent: Artifact = { 
      id: newId, 
      title: 'Advanced Module Analysis', 
      type: 'logic', 
      content: '', 
      status: 'streaming' 
    };
    ...
    try {
      ...
      const result = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Provide an additional specialized analysis for: ${session.prompt}. Think of something missing like Security, Auth, or a specialized Mobile view.`,
        config: { systemInstruction: "Generate a technical module analysis. Title the response clearly." }
      });
      ...
      setSession(prev => {
        ...
        return {
          ...prev,
          artifacts: prev.artifacts.map(a => 
            a.id === newId ? { ...a, content: result.text || '', status: 'complete' } : a
          )
        };
      });
    } catch (e) {
  ...
  const exportAllArtifacts = () => {
    ...
    fullExport += `Prompt: ${session.prompt}\n`;
    fullExport += `Generated on: ${new Date(session.timestamp).toLocaleString()}\n\n`;
    fullExport += `--- TECH STACK ---\n${projectMetadata?.stack.join(', ') || 'N/A'}\n\n`;
    
    session.artifacts.forEach(art => {
      fullExport += `\n========================================\n`;
      fullExport += `ARTIFACT: ${art.title}\n`;
      fullExport += `TYPE: ${art.type}\n`;
      fullExport += `========================================\n\n`;
      fullExport += art.content;
      fullExport += `\n\n`;
    });
    ...
    a.href = url;
    a.download = `${projectMetadata?.title?.toLowerCase().replace(/\s+/g, '-') || 'diffusion-project'}-export.md`;
          ...
          <div className="h-full flex flex-col items-center justify-center max-w-4xl mx-auto text-center px-4">
            <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
              ...
              <p className="text-lg md:text-xl text-gray-500 font-medium max-w-2xl mx-auto">
                An immersive Material workshop where AI agents transform your intent into architecture, code, and interfaces.
              </p>
          ...
          >
            ...
            {!focusedArtifactId && (
              ...
              >
                ...
                <p className="font-bold text-xs uppercase tracking-[0.2em] text-center">New Insight Agent</p>
                <p className="text-[10px] mt-1 text-gray-400 font-medium">Click to spawn more analysis</p>
              </div>
            )}
          </div>
        )}
      ...
      <footer className={`fixed bottom-0 sm:bottom-8 inset-x-0 z-[110] px-4 md:px-6 transition-all duration-500 ${isGenerating ? 'translate-y-24 opacity-0' : 'translate-y-0 opacity-100'}`}>
        <div className="max-w-4xl mx-auto mb-4 sm:mb-0">
          <div className="relative bg-white/95 backdrop-blur-2xl border border-gray-200 rounded-2xl sm:rounded-[2.5rem] p-1.5 md:p-2 pr-2.5 flex items-center shadow-[0_8px_32px_rgba(0,0,0,0.08)] focus-within:ring-4 focus-within:ring-blue-100 focus-within:border-[#1a73e8] transition-all">
            <div className="pl-4 md:pl-6 text-[#1a73e8] hidden xs:block">
            ...
            <input 
              ...
              placeholder={session ? "Refine your project..." : "Describe your software idea..."}
            ...
            <button 
              ...
              className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-full bg-[#1a73e8] text-white flex items-center justify-center hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all active:scale-95 disabled:bg-gray-100 disabled:shadow-none group shrink-0"
            >
              <Icons.Send className="w-4 h-4 md:w-6 md:h-6 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
            </button>
          </div>
        </div>
      </footer>
