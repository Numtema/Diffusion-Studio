...
export const ArtifactCard: React.FC<ArtifactCardProps> = ({ artifact, onFocus, onRemove, isFocused }) => {
    ...
    >
      ...
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-20">
        <div className="flex items-center gap-3">
          <div className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${isStreaming ? 'bg-[#1a73e8] animate-pulse shadow-[0_0_8px_rgba(26,115,232,0.4)]' : 'bg-gray-300 group-hover:bg-blue-400'}`} />
      ...
      {/* Surface Area */}
      <div className="flex-1 overflow-auto relative bg-gray-50/30 custom-scrollbar">
        {artifact.type === 'ui' ? (
              ...
              srcDoc={artifact.content || `
                <body style="margin:0;display:flex;align-items:center;justify-content:center;color:#9ca3af;font-family:sans-serif;height:100vh;background:#ffffff;">
                  <div style="text-align:center;">
                    <p style="font-weight:500;margin:0;">${isStreaming ? 'Generating Design...' : 'Canvas Ready'}</p>
                  </div>
                </body>
              `}
              title={artifact.id}
